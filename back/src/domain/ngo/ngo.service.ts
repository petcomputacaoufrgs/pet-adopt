...
export class NgoService {
  ...
  async getUnapproved() {
    ...
    const pendingUsers = await this.userService.getByRole(
  ...
  async getPage(filters: any = {}, approved: boolean = true) {
    ...
    Object.keys(restFilters).forEach((key) => {
      ...
      if (typeof restFilters[key] === 'string') {
        restFilters[key] = restFilters[key]
          .replace(/^"+|"+$/g, '')
          .replace(/^'+|'+$/g, '');
      }
    });
    ...
    const approvedUsers = await this.userService.getByRole(
      approved ? Role.NGO_ADMIN : Role.NGO_ADMIN_PENDING,
    );
    const ngoIds = approvedUsers.map((user) => user.ngoId);
    ...
    const [data, total] = await Promise.all([
      this.ngoModel.find(combinedFilters).skip(skip).limit(limit).exec(),
  ...
  async getById(id: string, includePrivateData: boolean = false) {
    const ngo = await this.ngoModel
      .findById(id)
      .select(includePrivateData ? undefined : '-doc');
    if (!ngo) {
  ...
  async update(id: string, updateNgoDto: UpdateNgoDto, session?: any) {
    const ngoUpdated = await this.ngoModel.findByIdAndUpdate(id, updateNgoDto, {
      new: true,
      session,
    });
  ...
  async approve(ngoId: string): Promise<Ngo> {
    ...
    try {
      ...
      await this.userService.updateUserRoleByNgoId(ngoId, 'NGO_ADMIN', session);
