...
export class PasswordService {
  ...
  validatePasswordStrength(password: string): void {
    const { valid, errors } =
      this.encryptionService.validatePasswordStrength(password);
    if (!valid) {
  ...
  async resetPassword(
    token: string,
    newPassword: string,
    revokeAllTokensFn: (userId: string) => Promise<void>,
    ...
    const hashedPassword =
      await this.encryptionService.encryptPassword(newPassword);
