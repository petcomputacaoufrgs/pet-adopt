...
export class AuthService {
  ...
  async validateUser(email: string, password: string): Promise<any> {
    ...
    if (user.role === Role.NGO_ADMIN_PENDING) {
      throw new ForbiddenException(
        'Aguardando aprovação da ONG pelos Admins do site',
      );
    }

    // Converter documento Mongoose para objeto plano e remover a senha
  ...
  async login(user: any, res: Response, deviceInfo?: string) {
    ...
    const { accessToken, refreshToken } =
      await this.tokenService.generateTokenPair(payload, deviceInfo);
  ...
  async revokeAllSystemTokens(): Promise<{
    message: string;
    ...
    return {
      message:
        'Todos os tokens do sistema foram revogados. Usuários precisarão fazer login novamente.',
      tokensRevoked: count,
    };
  }
  ...
  async updateNgoProfile(userId: string, updateDto: UpdateNgoDto) {
    ...
    if (!user.ngoId || user.role !== 'NGO_ADMIN') {
      throw new UnauthorizedException(
        'Apenas administradores de ONG podem atualizar',
      );
    }
